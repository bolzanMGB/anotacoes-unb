...
Antigamente os chips eram divididos em dois chips:

- North Bridge: Nela os barramentos eram de alta velocidade e conectavam a CPU à RAM e GPU/PCI.
- South Bridge: Nela os barramentos eram de baixa velocidade e conectavam periféricos de baixa velocidade, como USB, SATA, áudio, rede.

Hoje em dia:

- 1. PCs: CPU acessa diretamente a RAM e GPU e o Chipset assumiu a South Bridge.
- 2. SoC (smartphones): Único chip com vários tipos de barramentos internos..
...
- Definem a forma física (do sinal elétrico)de transmissão de bits via UART.
- Definem tensão, cabos, ruídos, distância.
- RS-232:
    - Ponto a ponto.
    - Único fio.
    - Full-duplex.
...
Se os Protocolos de Comunicação são o meio de envio através de cartas, os Protocolos de Aplicação seriam o formato da carta. Eles especificam:

- O formato em que os bits da mensagem são organizados.
- Comandos para leitura e escrita disponíveis. 
- Mecanismo de verificação de erro. 


### 7.1 Contexto da Automação Industrial

**1. MODBUS RTU:**

- Padrão na automação industrial.
- Formato: Endereço (1 byte) / Código da Função (1 byte) / Dados (tamanho variável) / CRC (2 bytes).
- CRC é um código de verificação de erro calculado a partir dos bytes da mensagem que é calculado antes da transmissão da transmissão e conferido no recibemento.
- CRC Lookup Table: tabela de consulta com possíveis valores pré-calculados para o CRC.

**2. PROFIBUS / PROFINET:** Alta performance.

**3. EtherCAT:** Em tempo real.

**4. OPC UA:** Alta segurança e confiabilidade.
...
**3. AMQP:** Mensageria robusta, comum em sistemas distribuídos.

### 7.3 Contexto da Internet e Redes

**1. HTTP / HTTPS:** Padrão.

**2. FTP:** Transferência de Arquivos.

**3. SMTP / POP / IMAP:** Email.
